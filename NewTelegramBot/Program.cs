...
using NewTelegramBot.extensions;
...
namespace NewTelegramBot
{
    class Program
    {
        private static readonly bool _requestApi = true;
        private static readonly string _geocoderApiKey = "";
        private static readonly string _weatherApiKey = "";
        private static readonly string _botApiKey = "";
        ...
        static async Task Main(string[] args)
        {
            ...
            menuManager.ApplyFor(privateCallbacks.CallbackManager);
            ...
            await BotBuilder.NewBuilder(_botApiKey)
        ...
        private static IMenuManager GetMenuManager()
        {
            ...
            var savedPage = new WidgetPage("saved", "Избранное", savedBody, new SavedFavoritsPage(OpenFollow));

            mainMenu.PathTo(savedPage);
            mainMenu.AddAction(StartSearching);

            menuManager.Define(mainPage);
            menuManager.Define(savedPage);
        ...
        private static async Task<string> GetWeatherInfo(double latitude, double longitude)
        {
            ...
            var weatherApiUrl = $"https://api.weather.yandex.ru/v2/forecast?" +
                    $"lat={latitude.ToString().Replace(',', '.')}&lon={longitude.ToString().Replace(',', '.')}";
            using (HttpClient client = new HttpClient())
            {
                client.DefaultRequestHeaders.Add("X-Yandex-API-Key", _weatherApiKey);
        ...
        {
           var menuManager = update.Owner.ResolveService<IMenuManager>();
           var page = menuManager.GetDefined("main");

           await menuManager.PushPageAsync(page, update);
        }
        ...
        private static async Task Do_InputCityAsync(SignedMessageTextUpdate update)
        {
            ...
            var geocoderApiUrl = $"https://geocode-maps.yandex.ru/1.x/?apikey={_geocoderApiKey}&geocode={cityName}&format=json";
            ...
            var resultMessage = $"Погода в запрошенном месте: {cityName} ({latitude.ToString().Replace(',', '.')}," +
        ...
        private static BotArgedCallback<GeoCoderInfo> FollowGeocode => 
            new(new LabeledData(
                "В избранное", "FollowGeocode"), Do_FollowGeocodeAsync);
        ...
        private static BotArgedCallback<IntWrapper> UnfollowGeocode => 
            new(new LabeledData(
                "Удалить", "UnfollowGeocode"), Do_UnfollowGeocodeAsync);
        ...
        private static BotArgedCallback<GeoCoderInfo> OpenFollow =>
            new(new LabeledData(
                "{ Открыть }", "OpenFollow"), Do_OpenFollowAsync);
        ...
        private static BotArgedCallback<GeoCoderInfo> LoadWeather =>
            new(new LabeledData("Узнать погоду", "LoadWeather"), Do_LoadWeatherAsync);
        ...
        private static async Task Do_LoadWeatherAsync(GeoCoderInfo args, SignedCallbackUpdate update)
        {
            if (update.Sender is BotUser user)
            {
                ...
                menu.Add(UnfollowGeocode,
                    new IntWrapper(user.Favs
                    .FindIndex(u => u.Name == args.Name)));
