...
namespace NewTelegramBot.Users
{
    internal class UserManager : IUsersManager
    {
        ...
        public async Task<IBotUser?> GetUserById(long telegramId) => 
            await Task.FromResult(
                Users.Find(x => x.TelegramId == telegramId));

        public async Task<bool> IsUserRegistered(long telegramId) => 
            await GetUserById(telegramId) is not null;
